  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-background">
        ...
        <p className="text-muted text-[14px] font-medium">Loading tasting...</p>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 flex flex-col bg-background safe-top safe-bottom">
        ...
        <div className="px-5 pt-5">
          <a href="/" className="inline-flex items-center gap-1 text-[13px] font-semibold text-muted hover:text-foreground transition-colors touch-target">
            <ChevronRight className="h-4 w-4 rotate-180" /> Leave
          </a>
        </div>

        <div className="flex-1 flex flex-col items-center justify-center px-6">
          <div className="animate-fade-in-up text-center">
            <div className="h-16 w-16 rounded-full bg-cherry/10 flex items-center justify-center mx-auto mb-5 animate-gentle-bob">
              <Wine className="h-8 w-8 text-cherry" />
            </div>
            <h1 className="text-[28px] font-bold font-serif text-foreground tracking-tight">
              {event.title}
            </h1>
            {event.description && (
              <p className="mt-2 text-muted text-[14px] max-w-sm mx-auto leading-relaxed">
                {event.description}
              </p>
            )}

            {/* Rotating tips */}
            <div className="mt-6 h-7 flex items-center justify-center">
              <p key={tipIndex} className="text-[13px] text-cherry/60 font-medium italic animate-fade-in-up">
                {LOBBY_TIPS[tipIndex]}
              </p>
            </div>

            <div className="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cherry/[0.06] text-cherry">
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              <span className="text-[13px] font-semibold">Waiting for host...</span>
            </div>
          </div>
          ...
          <div className="mt-10 w-full max-w-sm">
            ...
            <div className="bg-white rounded-[14px] border border-card-border/60 overflow-hidden divide-y divide-card-border/30">
              {event.guests.map((g: GuestParticipant) => {
                ...
                return (
                  <div key={g.id} className={`flex items-center gap-3 px-4 py-3 ${isMe ? "bg-cherry/[0.03]" : ""}`}>
                    <div className={`h-8 w-8 rounded-full flex items-center justify-center text-[12px] font-bold ${
                      isMe ? "bg-cherry text-white" : "bg-cherry/8 text-cherry"
                    }`}>
                      {g.displayName.charAt(0).toUpperCase()}
                    </div>
                    <span className={`text-[14px] ${isMe ? "text-cherry font-semibold" : "text-foreground font-medium"}`}>
                      {g.displayName}
                    </span>
                    {isMe && <span className="text-[10px] font-semibold text-cherry/50 ml-auto uppercase tracking-wider">You</span>}
                  </div>
                );
              })}
            </div>
    ...
    return (
      <div className="fixed inset-0 flex flex-col bg-background safe-top safe-bottom overflow-y-auto">
        <div className="px-4 md:px-8 lg:px-10 pt-6 pb-8">
          ...
          {/* Wine name card */}
          <div className="bg-white rounded-[14px] border border-card-border/60 p-6 mb-4 animate-scale-in">
            <h2 className="text-[22px] font-bold font-serif text-foreground tracking-tight mb-1">
          ...
          {currentGuess?.score != null && (
            <div className="bg-white rounded-[14px] border border-card-border/60 p-5 mt-4 flex items-center justify-between">
      ...
      <div className="px-5 pt-5 pb-2">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <h1 className="text-[24px] font-bold text-foreground tracking-tight mt-0.5">
              Wine <span className="nums">#{event.currentWine}</span>
            </h1>
          </div>
          <div className="h-10 w-10 rounded-full bg-cherry/10 flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth">
        <div className="px-5 pb-32 pt-2">
          {submitted && !editing ? (
            ...
            <div className="bg-white rounded-[14px] border border-card-border/60 overflow-hidden animate-scale-in">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-card-border/40">
                <div className="h-9 w-9 rounded-full bg-green-50 flex items-center justify-center">
                  <Check className="h-4 w-4 text-green-600" />
                </div>
                <div>
                  <span className="text-[15px] font-semibold text-foreground">Guess submitted</span>
                  <p className="text-[12px] text-muted">Waiting for the reveal...</p>
                </div>
              </div>

              <div className="px-5 py-3 divide-y divide-card-border/30">
              ...
              {!isWineLocked && (
                <div className="px-5 py-3 border-t border-card-border/40">
                  <button
                    ...
                    className="flex items-center gap-2 text-cherry font-semibold text-[13px] touch-target"
                  >
